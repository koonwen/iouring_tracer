...
** <2024-05-23 Thu>
  - [ ] Improving tracing
    - [ ] Fix nested calls (Needs separate tracks)
...
- [ ] Improving trace visualization
  - [ ] Understand and write about how uring works (detailing what happens exactly in the kernel)
  - [ ] Uring can have parallel syscalls in flight, figure out how to account for this
  - [ ] Fix nested calls
    - [ ] Add support for FXT async/flow events and see how they display in perfetto
    - [ ] Use separate tracks
  - [ ] Dropped events
...
asynchronous API's to IO.


At the heart of it, there are two ring buffers which are shared by the
kernel and the user application. One ring is called the submission
queue ring where the user application puts IO requests for the kernel
to consume. The other is called the completion queue ring which is
where the kernel puts the results of the requests and is consumed by
the application.

Between the time the kernel consumes the requests and
returns the results, io-uring makes various 'smart' decisions on how
your IO should be handled asynchronously. This is what I will focus on.
...
Define separate tracks as Submission ring track and Completion ring
track? Which means you need to define two kernel objects and link requests appropriately

** BPF ring buffer
- We could be dropping events not because of there is no space but
  because of high contention. Ringbuf should be able to support 1Mil ops/s
- ADd a counter to see exactly how many events we are dropping.

bpf-helpers.h are for kernel programming
bpf/libbpf.h and those annotated with LIBBPF are exposed to userspace for use
