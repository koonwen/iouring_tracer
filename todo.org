...
- [ ] Try on simple custom event ANS: You can see the custom events
  only when no GC is triggered. (Maybe the runtime buffer is dropping
  events?)  Yes you can see it when there's only a few events.

- [ ] Look through olly implementation. What runtime buffer is being
  recorded? Does it track different
threads
