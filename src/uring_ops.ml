...
let op_tbl : (unit Ctypes_static.ptr, 'a) Hashtbl.t = Hashtbl.create 50
...
let handle_event _ctx data _size fxt =
  ...
  | D.SYS_ENTER_IO_URING_ENTER
    ...
    (* W.duration_begin fxt ~name:"IO_URING_ENTER" ~thread ~category:"bpf" ~ts *)
  | D.SYS_EXIT_IO_URING_ENTER
    (* let pid = getf event D.pid |> Int64.of_int in *)
    (* let tid = getf event D.tid |> Int64.of_int in *)
    (* let thread = { Fxt.Write.pid; tid } in *)
    (* W.duration_end fxt ~name:"IO_URING_ENTER" ~thread ~category:"bpf" ~ts *)
    ->
      ()
  ...
  | D.IO_URING_COMPLETE -> (
      ...
      let req = getf t S.req in
      (* This argument is used to match start and end with a unique identifier *)
      let correlation_id = raw_address_of_ptr req |> Int64.of_nativeint in
      match Hashtbl.find_opt op_tbl req with
