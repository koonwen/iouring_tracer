...
let metadata ~ty ~data t =
  record t ~words:1 ~data:((data <<< 4) ||| i64 ty) ~ty:0

let traceinfo ~ty ~data t =
  metadata t ~ty:4 ~data:((data <<< 4) ||| i64 ty)

let magic_number t =
  traceinfo t ~ty:0 ~data:0x16547846L
...
module String_ref = struct
  ...
  let encode = function
    | Ref x -> x
    | Inline s -> 0x8000 lor (String.length s)
...
module Args = struct
  let add t = List.iter (fun (k, v) -> String_ref.add t k; Arg.add t v)
  let lookup t = List.map (fun (k, v) -> (String_ref.lookup t k, Arg.lookup t v))
  let words = List.fold_left (fun acc (k, v) -> acc + 1 + String_ref.words k + Arg.words v) 0
...
let event ~ty ?(args=[]) t ~name ~thread ~category ~ts =
  ...
  let words =
    2
    + Thread_ref.size thread
    + String_ref.words name
    + String_ref.words category
    + Args.words args
  in
  record t ~ty:4 ~words ~data:(
    i64 ty |||
    (i64 n_args <<< 4) |||
...
let instant_event  = event ~ty:0
let duration_begin = event ~ty:2
let duration_end   = event ~ty:3
...
let user_object ?(args=[]) t ~name ~thread id =
  ...
  let name = String_ref.lookup t name in
  let thread = Thread_ref.lookup t thread in
  let words = 2 + String_ref.words name + Thread_ref.size thread + Args.words args in
  record t ~ty:6 ~words ~data:(
...
let of_writer w =
  let t = {
    w;
    strings = String_ref.create ();
    threads = Thread_ref.create ();
  } in
  magic_number t;
  t
