...
let event ~ty ?correlation_id ?(args = []) t ~name ~thread ~category ~ts =
...
let flow_begin ~correlation_id = event ~ty:8 ~correlation_id
let flow_step ~correlation_id = event ~ty:9 ~correlation_id
let flow_end ~correlation_id = event ~ty:10 ~correlation_id
